class Mesa < Formula
  ...
  head "https://gitlab.freedesktop.org/lopin/mesa.git", branch: "darwin_wayland"
  ...
  def install
    ...
    # Workaround for build with wayland support
    env_vars = %w[HOMEBREW_INCLUDE_PATHS HOMEBREW_ISYSTEM_PATHS]
    ENV.remove env_vars, /(^|:)#{Regexp.escape(HOMEBREW_PREFIX)}\/include[^:]*/
